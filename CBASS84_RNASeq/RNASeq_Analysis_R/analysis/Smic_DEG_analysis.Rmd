...
conds2 <- read_csv(file="data/DESeq_Re-run_Conditions_CRVOct27.csv", col_names = T) %>%
...
conditions <- conds2$Conditions2 # Reverse comparison
```

# DEG analysis for Smic {.tabset}
...
```
<p>DEG Results for all conditions are [`r paste("output/", org, "/DEG_tables_svalues/", sep="")`](`r paste("output/", org, "/DEG_tables_svalues/", sep="")`)</p>
...
```
<p>Volcano plots for all conditions are [`r paste("output/", org, "/volcano_plots_svalues/", sep="")`](`r paste("output/", org, "/volcano_plots_svalues/", sep="")`)</p>
...
so.smic <- sleuth_prep(meta_data, num_cores = 8)
sleuth_matrix_smic <- sleuth_to_matrix(so.smic, 'obs_norm', 'tpm')
smic_TPM_matrix <- sleuth_matrix_smic[grep("Smic", row.names(sleuth_matrix_smic)),]

...
prepare_DESeq2_data <- function(wd="/Volumes/omics4tb2/Collaborations/Vulcan/R_Projects/Stylophora/",
  ...
  # Seperate Smic vs Smic
  dds.Smic <- ddsHTSeq[grep("Smic", rownames(ddsHTSeq)),]
  dds.Smic$condition <- relevel(dds.Smic$condition, ref = "30_KAUST")
  dds.Smic <- DESeq(dds.Smic)
  vsd.Smic <- vst(dds.Smic, blind = F)
  return(list(meta_data=meta_data, dds = dds.Smic, vsd= vsd.Smic, sample_table= sampleTable, directory=directory))
}
...
prepare_DESeq2_data_kaust <- function(wd="/Volumes/omics4tb2/Collaborations/Vulcan/R_Projects/Stylophora/",
  ...
  # Seperate Smic vs Smic
  dds.Smic <- ddsHTSeq[grep("Smic", rownames(ddsHTSeq)),]
  dds.Smic <- dds.Smic[,grep("Eilat", dds.Smic$Reef.Site.Name, invert=T)]
  dds.Smic$condition <- droplevels(dds.Smic$condition)
  dds.Smic$condition <- relevel(dds.Smic$condition, ref = "30_KAUST")
  dds.Smic <- DESeq(dds.Smic)
  vsd.Smic <- vst(dds.Smic, blind = F)
  return(list(meta_data=meta_data, dds = dds.Smic, vsd= vsd.Smic, sample_table= sampleTable, directory=directory))
...
####################
# loop through conditions to create DESeq analysis
make_deg <- function(condition=condition, dds=dds.Smic, lfc=2, write2file=T, sampleTable=sampleTable, directory=deseq_data$directory,svalue=0.005){
  dds.Smic <- dds
  ...
  dds.Smic$condition <- relevel(dds.Smic$condition, ref = cond2)
  
  # Run DEseq2
  dds.Smic <- DESeq(dds.Smic, parallel=F, BPPARAM=MulticoreParam(2))
  
  # get results
  #res <- results(dds.Smic, name = condition, alpha = 0.05)
  # # get results and use lfc shrinkage for visualization
   resLFC <- lfcShrink(dds = dds.Smic,
                      coef = condition,
                      type="apeglm",
                      lfcThreshold = lfc)
  ...
  if(write2file){
    write.table(as.data.frame(res.ordered), 
                file=paste("../RNA-seq_Analysis/Smic_DEG/DEG_tables_svalues/",condition,".txt", sep=""), sep="\t")
  }
...
####################
# loop through conditions to create DESeq analysis for temperature (Optional)
make_deg_temp <- function(sampleTable=deseq_data$sample_table, directory=deseq_data$directory, lfc=2, svalue=0.005, write2file=T){
  ...
  # Seperate Smic vs Smic
  dds.Smic <- ddsHTSeq[grep("Smic", rownames(ddsHTSeq)),]
  dds.Smic$Temp <- relevel(dds.Smic$Temp, ref = "33")
  dds.Smic <- DESeq(dds.Smic)
  vsd.Smic <- vst(dds.Smic, blind = F)
  
  
  for(condition in resultsNames(dds.Smic)[2:length(resultsNames(dds.Smic))]){
    # open pdf for volcano plot
    pdf(file = paste("../RNA-seq_Analysis/Smic_DEG/volcano_plots_svalues/", condition, ".pdf", sep=""))
...
make_deg_site <- function(sampleTable=deseq_data$sample_table, directory=deseq_data$directory, lfc=2, svalue=0.005, write2file=T){
  ...
  # Seperate Smic vs Smic
  dds.Smic <- ddsHTSeq[grep("Smic", rownames(ddsHTSeq)),]
  dds.Smic$Reef.Site.Name <- relevel(dds.Smic$Reef.Site.Name, ref = "KAUST")
  dds.Smic <- DESeq(dds.Smic)
  vsd.Smic <- vst(dds.Smic, blind = F)
...
volcano_plot <- function( condition = condition, results_dir="../RNA-seq_Analysis/Smic_DEG/DEG_tables_svalues/", lfc=2, svalue=0.005){
  # what analysis is being performed
...
###################
# function to create a matrix of DEG numbers across comparisons
create_DEG_matrix <- function(p.value=0.005){
  setwd("/Volumes/omics4tb2/Collaborations/Vulcan/R_Projects/Stylophora/")
  # counts directory
  directory <- "../../R_Projects/RNA-seq_Analysis/Smic_DEG/DEG_tables_svalues/"
...
plot_heatmap <- function(hm_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_heatmap.pdf", dds=deseq_data$dds){
  # 1. heatmap
  pdf(file = hm_file)
  dds.Spis <- dds
  ...
  pheatmap(df.order, scale = "none", cluster_rows=F, border_color = NA, show_rownames=TRUE, # heatmap
           cluster_cols=F, annotation_col=df.annotate, main = "Smic Top 30 DEG", fontsize_col = 7, fontsize_row = 6, color = viridis(20) )
  dev.off()
}
...
###################
# 2. PCA Plot
plot_pca <- function(pca_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_PCA.pdf", vsd=deseq_data$vsd){
  pdf(file = pca_file )
  vsd.Smic <- vsd
  # remove outlier ES1-36A and create PCA object
  pcaData <- plotPCA(vsd.Smic[,grep("ES1-36A", colnames(vsd.Smic), invert = T)], intgroup=c("Temp","Reef.Site.Name","condition"), returnData=TRUE)
...
###################
# 2.1 PCA Plot with KAUST only
plot_pca_kaust <- function(pca_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_PCA_KAUST.pdf", vsd=deseq_data_kaust$vsd){
  pdf(file = pca_file )
  vsd.Smic <- vsd
  # remove outlier ES1-36A and create PCA object
  pcaData <- plotPCA(vsd.Smic[,grep("Eilat", vsd.Smic$Reef.Site.Name, invert = T)], intgroup=c("Temp","Reef.Site.Name","condition"), returnData=TRUE)
  percentVar <- round(100 * attr(pcaData, "percentVar"))
  q <- ggplot(pcaData, aes(PC1, PC2, color=Temp, shape=Reef.Site.Name)) +
    geom_point(size=4, alpha=0.7) +
...
###################
# 3. distance heatmap
plot_dist_heatmap <- function(dhm_file = "../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_distance_heatmap.pdf", dds=deseq_data$dds, vsd=deseq_data$vsd){
  pdf(file=dhm_file)
  dds.Smic <- dds
  vsd.Smic <- vsd
  df <- as.data.frame(colData(dds.Smic)[,c("Temp","Reef.Site.Name","Site","condition")])
  rownames(df) <- paste(vsd.Smic$Temp,vsd.Smic$Reef.Site.Name,vsd.Smic$sample, sep = "_")
  sampleDists <- dist(t(assay(vsd.Smic)))
  sampleDistMatrix <- as.matrix(sampleDists)
  rownames(sampleDistMatrix) <- paste(vsd.Smic$Temp,vsd.Smic$Reef.Site.Name,vsd.Smic$sample, sep = "_")
...
###################
# 4. DEG Barplot
plot_barplot <- function(barplot_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_barplot.pdf", vsd=deseq_data$vsd, df=DEG.matrix$updown){
  ...
  p1 <- p1 + labs(x="Comparison", y="# of DEG", title="Smic DEG for all comparisons")
  ...
  p2 <- p2 + labs(x="Comparison", y="# of DEG", title="Smic DEG for all comparisons")
  ...
  p3 <- p3 + labs(x="Comparison", y="# of DEG", title="Smic DEG for all comparisons")
  ...
  p4 <- p4 + labs(x="Comparison", y="# of DEG", title="Smic DEG for all comparisons")
...
###################
# test to see optimal number of replicates
replicate_test <- function(plot=T, sampleTable=deseq_data$sample_table, directory=directory){
  ...
  for(replicate_no in 2:7){
    for(run in 1:500){
      ...
      # Run DEseq2
      dds.Smic <- DESeq(dds.Smic, parallel=F)
...
###################
# function to perform k-means clustering on differentially expressed genes
kmeans_clustering <- function(km_file="../RNA-seq_Analysis/Smic_DEG/Smic_STAR_DEG_kmeans_plots.pdf", vsd=deseq_data$vsd){
  
  pdf(file=km_file)
  vsd.Smic <- vsd
  # get sample names from all directories for Stylophora
  samples <- list.files("../../R_Projects/RNA-seq_Analysis/Smic_DEG/DEG_tables/", recursive = T, pattern="(*_vs_30_KAUST*)", full.names = T)
  ...
  vsd_matrix <- assay(vsd.Smic)[all_diff,]
  
  vsd.Smic <- as.data.frame(vsd_matrix)
  distance <- get_dist(t(vsd.Smic))
  fviz_dist(distance, gradient = list(low = "#00AFBB", mid = "white", high = "#FC4E07"))
  # determine optimal number of clusters
  set.seed(123)
  ## high efficiency clustering
  res.km <- eclust(vsd.Smic, "kmeans", nboot = 500)
  fviz_silhouette(res.km, print.summary = T)
  
  fviz_nbclust(vsd.Smic, kmeans, method = "wss") # find optimal number of clusters
  # run k-means
  Smic.k <- kmeans(vsd.Smic, centers = res.km$nbclust, nstart = 25)
  fviz_cluster(Smic.k, data = vsd.Smic, geom = "point", main = "Smic kmeans Clustering")
  write.table(data.frame(Smic.k$cluster), sep="\t", file="../RNA-seq_Analysis/Smic_DEG/Smic_STAR_DEG_kmeans.txt")
  dev.off()
}


###################
## function to create conditions when run is half completed
conditions_notcomplete <- function(){
  # create all conditions for all combinations
  conds <- unique(meta_data$condition)
  
  # check the folder to see which comarison have already been done.
  done_comparisons <- dir(file.path("../../R_Projects/Stylophora/Smic_DEG/"))
  done_comparisons <- done_comparisons[!(done_comparisons %in% c("old"))]
  done_comparisons <- sub(".txt", "", done_comparisons)
  
  # create conditions for comparison if run was not completed previously
  conditions <- vector()
  # loop through conditions to create DESeq analysis
  for(cond1 in conds){
    for(cond2 in conds){
      if(cond1 != cond2){
        ...
        # create comparison name for results
        name = paste("condition_", cond2, "_vs_", cond1, sep="")
        
        # check if the comparison already exists
        if(!(name %in% done_comparisons)){
          conditions <- append(name, conditions)
        }
      }
    }
...
##### TopGO Enrichment Functions #####
##### create gene2go object for function enrichment #####
gene2go.object <-function(file=NULL){
  file = "/Volumes/omics4tb2/Collaborations/Vulcan/Genomics/algae_Symbiodinium/smic.genome.genes.annotation_clean.txt"
...
for(condition in conditions){
  ...
  # volcano plot
  pdf(file=paste("../RNA-seq_Analysis/Smic_DEG/volcano_plots_svalues/",condition,"_volcano.pdf", sep=""))
  volcano_plot(condition = condition, results_dir="../RNA-seq_Analysis/Smic_DEG/DEG_tables_svalues/", lfc=2, svalue=0.005)
  dev.off()
}
toc()
...
write.table(DEG.matrix$matrix, file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_matrix.txt", sep="\t")
write.table(DEG.matrix$updown, file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_matrix_up and_down.txt", sep="\t")

pdf(file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_matrix_heatmap.pdf")
pheatmap(mat=DEG.matrix$matrix, cluster_rows = F, cluster_cols = F, display_numbers = T, number_format = "%.0f", main ="Smic DEG Matrix")
dev.off()

plot_barplot(barplot_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_barplot.pdf")

# 5. heatmap
plot_heatmap(hm_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_heatmap.pdf")

# 6. PCA
plot_pca(pca_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_PCA.pdf")

# 6.1 PCA for KAUST Sites only
plot_pca_kaust(pca_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_PCA_KAUST.pdf", vsd=deseq_data$vsd)

# 7. distance heatmap
plot_dist_heatmap(dhm_file = "../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_distance_heatmap.pdf")

# 8. replicate test
replicate_profiles <- replicate_test(sampleTable = deseq_data$sample_table, directory = deseq_data$directory)
write.table(replicate_profiles, file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_replicates_test_36KAUST_vs_30KAUST.txt", sep="\t")

# 9. Kmeans clustering of differentially expressed genes
kmeans_clustering(km_file="../RNA-seq_Analysis/Smic_DEG/Overall_Plots_svalues/Smic_STAR_DEG_kmeans_plots.pdf", vsd=deseq_data$vsd)
...
# 12. TopGO Enrichment
Smic.enrichment <- run.topGO.enrichment(DEG.matrix$deg_genes)
names(Smic.enrichment) <- names(DEG.matrix$deg_genes)
Smic.go.enrichment <- post.process(Smic.enrichment)
write.table(Smic.go.enrichment, file="../RNA-seq_Analysis/Smic_DEG/Smic_DEG_GO_Enrichment.txt", sep="\t", row.names=F)

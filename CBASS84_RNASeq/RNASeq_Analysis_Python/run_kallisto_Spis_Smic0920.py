...
genome_dir = "%s/reference0920" %run_dir
transcriptome_file = "%s/Spis.Smic.genes.nt.longest.fa" %genome_dir
...
def run_kallisto(first_pair_group,second_pair_group,results_dir,kallisto_input_files):
    ...
    kallisto_cmd = '/users/sturkars/kallisto/kallisto quant -i %s/Spis_Smic_transcripts_kallistoindex %s -o %s  -b 100 --bias -t 4 --rf-stranded' %(genome_dir, kallisto_input_files, results_dir)
...
def run_pipeline():
    ...
    results_dir = "%s/%s/results0920_kallisto_Spis_Smic" %(data_dir,folder_name)

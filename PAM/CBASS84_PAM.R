...
#modified by Chris Voolstra
setwd("../CBASS84/")
library(emmeans)
library(drc)

#### ReadIn/QAQC ####
pamdata<-read.delim("../CBASS84/CBASS84_PAM_data.txt")
...
write.table(data.frame("Genet"=gsub("ed50:", "", row.names(GenoCoeffs)), GenoCoeffs), file="../CBASS84/GenetED50s.txt", quote=F, sep="\t", row.names=F)
write.table(SumaryStats, file="../CBASS84/CBASS84_ED50_SummaryStats.txt", quote=F, sep="\t", row.names=F)
...
pdf("../CBASS84/CBASS84_DRCCurves_MMMs.pdf",10,7)
line_width=2
offsets<-c(0.1875,0.0625,-0.0625,-0.1875)
i<-1 #ICN
matplot(temp_x, predict(DRCpamICN, data.frame(Temp = temp_x), interval="confidence"),
